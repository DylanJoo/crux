...
def main(
    ...
    output_path = os.path.join(
        args.output_dir, 
        f"ratings.{split}.{args.model.split('/')[-1]}.{args.shard}-{args.total_shards}.jsonl"
    )
    if os.path.exists(output_path.replace("-offload", "")): # remove the offload for original dataset
        ratings_done = load_ratings(output_path.replace("-offload", ""))
    else:
