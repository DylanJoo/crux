...
import ir_measures
from ir_measures import Metric, MAP, nDCG, P
import pdb

# NOTE: integrate to ir_measures
def coverage_meausres(ratings, ratings_oracle, topk=10, filter_by_oracle=False, tau=3):
...
def rac_eval(run, qrel, judge, tau=3, filter_by_oracle=False, run_b=None):
    ...
    for qid in run: 
        ...
        ratings = np.max([judge[qid][docid] for docid in run[qid] if docid in judge[qid]], 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filter_by_oracle", type=str, default=None)
    ...
    # run eval
    outputs = rac_eval(run=run, qrel=qrel, run_b=runb, judge=ratings, filter_by_oracle=False, tau=3)

    # Get evearge across queries
    for key in outputs:
        print(key, np.mean(outputs[key]))
