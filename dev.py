...
def rac_eval(run, qrel, div_qrel, judge, tau=3, filter_by_oracle=False, run_b=None):
    ...
    for qid in run: 
        ...
        ratings = np.max(empty + [judge[qid][docid] for docid in run[qid] if docid in judge[qid]], 0)
...
if __name__ == "__main__":
    ...
    # run eval
    outputs = rac_eval(run=run, qrel=qrel, div_qrel=div_qrel, run_b=runb, judge=ratings, filter_by_oracle=args.filter_by_oracle, tau=3)
